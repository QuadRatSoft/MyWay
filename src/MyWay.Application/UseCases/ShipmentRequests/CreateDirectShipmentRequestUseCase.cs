...
public sealed class CreateDirectShipmentRequestUseCase
{
    ...
    public CreateDirectShipmentRequestUseCase(
        ICurrentUserContext currentUserContext,
        IUserAccessService userAccessService,
        IShipmentRequestRepository shipmentRequestRepository,
        ICarrierListingRepository carrierListingRepository,
    ...
    public async Task<Result<Guid>> ExecuteAsync(
        CreateDirectShipmentRequestCommand command,
        CancellationToken cancellationToken = default)
    {
        ...
        var carrierListing = await carrierListingRepository.GetByIdAsync(
            command.TargetCarrierListingId,
            cancellationToken);

        if (carrierListing is null)
        {
            return Result<Guid>.Failure(CarrierListingErrors.NotFound(command.TargetCarrierListingId));
        }

        if (!carrierListing.IsVisibleOnCarrierBoard)
        {
            return Result<Guid>.Failure(CarrierListingErrors.NotAvailable(command.TargetCarrierListingId));
        }
